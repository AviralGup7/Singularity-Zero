...
def _extract_stage_name(event: PipelineEvent) -> str:
    """Extract stage name from event source (e.g. 'stage.subdomains' -> 'subdomains')."""
...
def register_event_metrics_subscribers(event_bus: EventBus) -> None:
    ...
    pipeline_success_counter = metrics.counter(
    ...
    def _on_pipeline_complete(event: PipelineEvent) -> None:
        if event.data.get("_telemetry_bound"):
            return
        pipeline_success_counter.inc()
    ...
    event_bus.subscribe(EventType.PIPELINE_COMPLETE, _on_pipeline_complete)
