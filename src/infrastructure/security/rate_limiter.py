...
class SlidingWindowCounter:
    ...
    def increment(self, key: str, limit: int) -> RateLimitResult:
        ...
        with self._lock:
            ...
            # Weighted sliding window: previous_count * (1 - position) +
            # current_count. Using only current_count doubled the limit
        ...
        if self._redis_counter is not None:
            try:
                ...
                try:
                    ...
                    logger.warning(
                        "RateLimiter: Failed to record Redis fallback metric", exc_info=True
                    )
