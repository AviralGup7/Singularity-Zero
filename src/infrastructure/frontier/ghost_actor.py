...
# Migration timeout: if a migration takes longer than this, auto-abort.
MIGRATION_TIMEOUT_SECONDS: float = 30.0


def _unregister_logic(name: str) -> None:
    """Remove a logic function from both weak and strong registries."""
    _LOGIC_REGISTRY.pop(name, None)
    _LOGIC_STRONG_REFS.pop(name, None)
...
class ScanActor(pykka.ThreadingActor):
    ...
    def __init__(
        ...
        self._migration_started_at: float = 0.0
    ...
    def on_receive(self, message: dict[str, Any]) -> Any:
        ...
        # Auto-abort stuck migrations (Bug #7 fix)
        if (
            self.is_migrating
            and self._migration_started_at > 0
            and time.time() - self._migration_started_at > MIGRATION_TIMEOUT_SECONDS
        ):
            logger.warning(
                "Ghost-Actor [%s]: Migration timed out after %.1fs, auto-aborting",
                self.actor_id,
                MIGRATION_TIMEOUT_SECONDS,
            )
            self.is_migrating = False
            self._migration_started_at = 0.0

        # Explicit command allowlist — unknown commands are rejected, not silently accepted
        known_commands = {
            "execute",
            "snapshot",
            "recover",
        ...
        if command == "execute":
            ...
            self.is_migrating = True
            self._migration_started_at = time.time()
    ...
    def _unregister_self(self) -> None:
        """Remove this actor's logic function from the strong-ref registry.

        Called when the actor stops to prevent unbounded memory growth
        from stale entries in _LOGIC_STRONG_REFS (Bug #6 fix).
        """
        name = getattr(self.logic_fn, "__name__", "")
        if name:
            # Only unregister if no other live actor holds the same logic fn.
            # We can't check here, so we do a best-effort: only remove from
            # _LOGIC_STRONG_REFS. _LOGIC_REGISTRY is WeakValueDictionary and
            # self.logic_fn is still referenced by self, so it won't be GC'd yet.
            _LOGIC_STRONG_REFS.pop(name, None)

    def stop(self, *args: Any, **kwargs: Any) -> None:
        """Override stop to clean up logic registry before actor shutdown."""
