...
def cache_deception_detector(
    ...
    for response in responses:
        ...
        resp_signals.extend(_check_cache_headers(response))
        resp_signals.extend(_check_authenticated_endpoint_context(url, response))
        ...
        if "auth_endpoint_publicly_cacheable" in resp_signals:
            resp_title_parts.append("Auth Endpoint Publicly Cacheable")
        if "sensitive_endpoint_publicly_cacheable" in resp_signals:
            resp_title_parts.append("Sensitive Endpoint Publicly Cacheable")
        if "public_cache_directive" in resp_signals:
            resp_title_parts.append("Public Cache Directive")
        if "missing_vary_header" in resp_signals:
            resp_title_parts.append("Missing Vary Header")
        if "sensitive_endpoint_missing_vary" in resp_signals:
            resp_title_parts.append("Sensitive Endpoint Missing Vary")
        if "long_cache_max_age" in resp_signals:
            resp_title_parts.append("Long Cache Max-Age")

        title = (
            "; ".join(resp_title_parts)
            if resp_title_parts
            else "Cache Deception Response Indicator"
        )
