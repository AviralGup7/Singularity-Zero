...
class ReportTemplate:
    ...
    def format_hackerone(finding: dict[str, Any], program: str) -> dict[str, Any]:
        return {
            "program": program,
            "vulnerability_type": str(finding.get("vuln_type") or finding.get("category") or finding.get("title") or "Other"),
            "title": str(finding.get("title") or finding.get("name") or finding.get("vuln_type") or "Untitled Report"),
            "description": str(finding.get("description") or finding.get("summary") or ""),
            "severity": str(finding.get("severity") or "medium").lower(),
            "target_url": str(finding.get("target_url") or finding.get("affected_url") or finding.get("url") or ""),
            "affected_urls": list(
                {u.strip() for u in (finding.get("affected_urls") or [finding.get("affected_url") or finding.get("url", "")]) if u.strip()}
            ),
            "evidence": dict(finding.get("evidence") or {}),
            "cvss_score": finding.get("cvss_score"),
            "cwe_id": finding.get("cwe_id"),
            "references": list(finding.get("references") or []),
            "reproduction_steps": str(finding.get("reproduction_steps") or finding.get("steps_to_reproduce") or ""),
        }
