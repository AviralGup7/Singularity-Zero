...
def cookie_security_checker(responses: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for response in responses:
        ...
        for cookie in raw_cookie_headers:
            ...
            has_expires = "expires=" in lowered or "max-age=" in lowered
            if not has_secure and not has_httponly and not has_samesite:
                if not has_expires or any(
                    m.group(0)
                    for m in re.finditer(r"max-age=(\d+)", lowered, re.IGNORECASE)
                    if int(m.group(1)) > 86400
                ):
                    issues.append(f"session_cookie_insecure:{name}")
