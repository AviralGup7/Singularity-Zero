...
class CircuitBreaker:
    ...
    def _try_admit_call(self) -> tuple[bool, str, int, int]:
        with self._lock:
            ...
            if self.state == "OPEN" and now - self.last_state_change >= self.recovery_timeout:
                ...
                self._half_open_probe_in_flight = False
                logger.info(
                    "CircuitBreaker [%s]: Entering HALF_OPEN state (trial=%d).",
                    self.name,
                    self._trial_generation,
                )

            if self.state == "OPEN":
                return False, self.state, self._state_version, self._trial_generation
            ...
            return True, self.state, self._state_version, self._trial_generation

    def _record_success(self, admission_state: str, state_version: int, trial_gen: int) -> None:
        with self._lock:
            if self._state_version != state_version or self._trial_generation != trial_gen:
                return
    ...
    def _record_failure(
        self, admission_state: str, state_version: int, trial_gen: int, exc: Exception
    ) -> None:
        with self._lock:
            if self._state_version != state_version or self._trial_generation != trial_gen:
                return
    ...
    def _release_aborted_probe(
        self, admission_state: str, state_version: int, trial_gen: int
    ) -> None:
        with self._lock:
            if (
                ...
                and self._trial_generation == trial_gen
            ):
                self._half_open_probe_in_flight = False
