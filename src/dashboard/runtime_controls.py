...
Defines NumericControlSpec and ToggleControlSpec dataclasses for building
dashboard form controls, plus utilities for applying runtime overrides
to pipeline configuration and building form defaults.
"""
...
RUNTIME_NUMERIC_CONTROLS: list[NumericControlSpec] = [
    NumericControlSpec(
        ...
        100,
        maximum=500,
        default_resolver=lambda config: str(_resolve_priority_limit_default(config)),
    ),
    NumericControlSpec(
        "max_collected_urls",
        "Max Collected URLs",
    ...
    NumericControlSpec(
        "screenshot_max_hosts",
        "Screenshot Hosts",
        ("screenshots", "max_hosts"),
        12,
        maximum=100,
    ),
    NumericControlSpec(
