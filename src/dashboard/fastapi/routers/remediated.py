...
async def verify_finding_remediation(
    ...
    # 2. Locate finding by ID, scoped to the current tenant.
    # SECURITY: pass ``tenant_id`` so the lookup returns nothing for
    ...
    # 3. Enforce Tenant Boundaries BEFORE the file scan. Iterating all
    # targets before the tenant check leaks the *existence* of findings
    # in other tenants through response timing. The check must run first.
