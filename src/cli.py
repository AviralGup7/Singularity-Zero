...
def handle_plugin_new(args: argparse.Namespace) -> int:
    ...
    if category == "recon":
        ...
        # Scaffolding content. Recon sources in this project are
        # *function-based* (see ``rapiddns.py``/``virustotal.py``), not
        # classes deriving from a non-existent ``BaseReconSource``.
