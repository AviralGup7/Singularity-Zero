...
from src.intelligence.swarm.p2p.consensus import NoiseChannel, BFTMessageValidator
...
class AgentNode:
    ...
    def discover_finding(self, finding: dict[str, Any]) -> None:
        ...
        logger.debug("Agent %s (%s) discovered finding: %s", self.node_id, self.role, finding.get("title"))
    ...
    def _export_bft_payload(self) -> bytes:
        ...
        state_dict = {
            "urls": list(self.get_known_urls()),
            "findings": self.get_known_findings()
        }
    ...
    def _import_bft_payload(self, encrypted_payload: bytes, shared_channel: NoiseChannel) -> None:
        ...
        try:
            ...
            if not BFTMessageValidator.verify_state(payload["state"], payload["signature"], public_key):
            ...
            for finding in payload["state"]["findings"]:
                self.state.apply_delta({"node_id": sender_id, "findings": [finding], "_ts": time.time()})
