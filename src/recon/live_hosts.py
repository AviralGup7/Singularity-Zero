...
def probe_host_without_httpx(host: str, timeout_seconds: int) -> dict[str, Any] | None:
    ...
    if "://" not in host:
        try:
            ...
            logger.warning("Operation failed in live_hosts.py: %s", exc, exc_info=True)  # noqa: BLE001
    ...
    for candidate, header_host in safe_candidates:
        try:
            ...
            # effectively a bare ``except Exception``. We now list the
            # specific urllib3/socket errors we actually want to swallow
            # so a programmer reading this knows the intent.
            continue
    return None
