...
class CloudBucketScanner:
    ...
    async def _probe_s3_website(
        ...
        regions = getattr(self, "s3_website_regions", None) or DEFAULT_S3_WEBSITE_REGIONS
    ...
    async def _probe_common_object_paths(
        ...
        from src.recon.cloud_recon.constants import _S3_COMMON_OBJECT_PATHS

        public_paths: list[str] = []
        paths = getattr(self, "s3_object_paths", None) or _S3_COMMON_OBJECT_PATHS
        for path in paths:
    ...
    def enumerate_cloud_run_candidates(self, target: str) -> list[str]:
        ...
        is a 9-12 character random string.
        """
        parsed = urlparse(target if "://" in target else f"https://{target}")
        domain = parsed.hostname or parsed.path or target
        core_name = domain.split(".")[0].lower().strip()
