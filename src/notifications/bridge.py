...
def connect(store: MemoryJobStore, inbox: Inbox) -> None:
    def _on_event(event: JobEvent) -> None:
        if event.type is JobEventType.COMPLETED:
            inbox.push(job_completed(event.job_id, event.payload.get("hostname", ""), int(event.payload.get("findings", 0) or 0)))
        elif event.type is JobEventType.FAILED:
            inbox.push(job_failed(event.job_id, str(event.payload.get("hostname") or ""), event.message))
