...
class StagePlanner:
    ...
    def plan_stages(self, remaining_stages: list[str]) -> tuple[list[str], dict[str, Any]]:
        ...
        if (
            ...
            logger.info(
                "StagePlanner: Findings count (%d) >= 50. Auto-injected threat_modeling enrichment.",
                findings_count,
            )
        ...
        if (
            has_js
            and "semgrep" not in adjusted_stages
            and "semgrep" not in getattr(result, "stage_status", {})
        ):
            adjusted_stages.append("semgrep")
            logger.info(
                "StagePlanner: Auto-inserted 'semgrep' stage due to discoverable Javascript URLs."
            )
        ...
        if (
            has_wildcard
            and len(subdomains) > 0
            and "subdomain_takeover" not in adjusted_stages
            and "subdomain_takeover" not in getattr(result, "stage_status", {})
        ):
            adjusted_stages.append("subdomain_takeover")
            logger.info(
                "StagePlanner: Auto-inserted 'subdomain_takeover' stage due to wildcards and enumerated subdomains."
            )
        ...
        if url_count > 0:
            ...
            resources["rate_limit_delay"] = (
                0.5 if url_count >= 50000 else (0.1 if url_count >= 5000 else 0.0)
            )
            logger.info(
                "StagePlanner: Calibrated resources for %d URLs: concurrency=%d, depth=%d",
                url_count,
                resources["httpx_concurrency"],
                resources["katana_depth"],
            )
            ...
            if waf_detected_count == 0:
                ...
                if hasattr(result, "stage_status"):
                    ...
                    result.module_metrics["waf"] = {
                        "status": "skipped",
                        "reason": "probabilistic_skip_low_confidence",
                    }
        ...
        for s, val in ordered_stages:
            if val < threshold:
                ...
                if hasattr(result, "module_metrics"):
                    result.module_metrics[s] = {
                        "status": "skipped",
                        "reason": f"low_marginal_value_{val:.2f}",
                    }
                continue
    ...
    def _sample_waf_detection(self, sample_urls: list[str]) -> int:
        ...
        try:
            ...
            logger.debug(
                "StagePlanner: WAF detector import unavailable; assuming 0 detections from sample."
            )
            return 0
