...
export function ComplianceDashboard() {
  ...
  const getMaturityColor = (maturity: string) => {
    switch (maturity) {
      case 'PASS': return 'text-ok border-ok/30 bg-ok/5';
      case 'PARTIAL': return 'text-warn border-warn/30 bg-warn/5';
      case 'AT_RISK': return 'text-orange-400 border-orange-500/30 bg-orange-500/5';
      case 'FAIL': return 'text-bad border-bad/30 bg-bad/5';
  ...
  const donutData = useMemo(() => {
    ...
    return [
      ...
      { name: 'At Risk', value: atRisk, color: 'var(--warn)' },
    ...
    >
      <PageHeader
        ...
        actions={
          <div className="flex items-center gap-3">
            <select 
              className="bg-surface-2 border border-line rounded-lg px-3 py-2 text-sm text-text-primary focus:border-accent focus:shadow-[0_0_0_2px_var(--accent-soft)] transition-all duration-200 cursor-pointer min-w-[200px]"
        ...
        <>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <GlassCard key={stat.name} variant="glow" delay={i * 0.05} className="space-y-3 flex flex-col justify-between">
                  <div className="space-y-2">
                    <div className="flex justify-between items-start">
                      <h3 className="text-xs font-bold uppercase tracking-wider text-text-secondary">{stat.name}</h3>
                      <span className={`text-lg font-mono font-bold ${stat.score > 80 ? 'text-ok' : stat.score > 50 ? 'text-warn' : 'text-bad'}`}>
                  ...
                  <div className="flex justify-between text-[10px] font-mono border-t border-line pt-2 mt-1">
                    <span className="text-ok font-semibold">{stat.passed} PASS</span>
                    <span className="text-bad font-semibold">{stat.failed} FAIL</span>
                    <span className="text-text-tertiary">{stat.total} TOTAL</span>
                  </div>
                </GlassCard>
              ))}
            </div>
            ...
            <GlassCard variant="glow" delay={0.2} className="flex flex-col items-center justify-center p-4">
              <h3 className="text-xs font-bold uppercase tracking-wider text-text-secondary mb-2 w-full text-left">Control Status Distribution</h3>
                    ...
                    >
                      ...
                      <AnimatePresence initial={false}>
                        {expandedControls.has(cid) && (
                          ...
                          >
                            <div className="p-4 pt-0 border-t border-line/50 bg-surface-2 space-y-4">
                              <div className="mt-4">
                                <h4 className="text-[10px] uppercase text-text-tertiary font-bold mb-1">Recommendation</h4>
                                <p className="text-xs italic text-text-secondary">{data.recommendation}</p>
                              </div>
                              
                              {data.findings.length > 0 && (
                                <div>
                                  <h4 className="text-[10px] uppercase text-text-tertiary font-bold mb-2">Evidence ({data.findings.length})</h4>
                                  <div className="space-y-2">
                                    {data.findings.map(f => (
                                      <div key={f.id} className="text-xs flex items-center justify-between bg-surface p-2 rounded border border-line">
                                        <div className="flex items-center gap-2">
                                          <span className={`w-2 h-2 rounded-full ${
                                            f.severity === 'critical' ? 'bg-bad' : 
                                            f.severity === 'high' ? 'bg-orange-500' : 'bg-warn'
                                          }`} />
                                          <span className="font-bold text-text-primary">{f.title}</span>
                                          <span className="text-text-tertiary truncate max-w-[300px]">{f.url}</span>
                                        </div>
      ...
      {report && report.total_findings === 0 && !loading && (
        <div className="card p-12 text-center space-y-4">
          <ShieldCheck size={48} className="mx-auto text-ok opacity-50" />
          <div>
