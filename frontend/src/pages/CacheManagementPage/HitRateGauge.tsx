...
export function HitRateGauge({ value }: { value?: number | null }) {
  ...
  return (
    <div className="flex items-center gap-4">
      ...
      >
        <div className="grid h-16 w-16 place-items-center rounded-full bg text-lg font-bold">
          {formatRatio(value)}
        </div>
      </div>
      <div className="min-w-0">
        <p className="font-mono text-xs uppercase tracking-wider text-muted">Hit Rate</p>
        <p className="mt-1 text-sm text-text">Miss rate {formatRatio(value === null || value === undefined ? null : 1 - value)}</p>
      </div>
    </div>
  );
}
