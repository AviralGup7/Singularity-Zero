...
function CockpitSetupViewBase({
  ...
  return (
    <div className="relative flex h-full w-full flex-col overflow-y-auto bg-bg p-8 cyber-grid-overlay scrollbar-cyber">
      ...
      <div className="relative z-10 m-auto flex w-full max-w-4xl flex-col items-center justify-center py-12">
        ...
        <div className="w-full rounded-2xl border border-line-strong bg-surface/85 p-6 shadow-panel-glass backdrop-blur-xl md:p-8">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="space-y-6">
              <div>
                <h3 className="font-mono text-xs font-bold uppercase tracking-wider text-text-primary mb-2 flex items-center gap-2">
                ...
                <input
                  ...
                  className="w-full rounded-lg border border-line bg-surface-2 px-4 py-3 font-mono text-xs text-text-primary placeholder:text-text-tertiary/40 outline-none focus:border-accent/40 focus:ring-1 focus:ring-accent/40 transition-all shadow-inner"
                  aria-label="Scan target URL"
                />
                <p className="mt-1.5 font-mono text-[9px] text-text-secondary leading-relaxed">
                      ...
                      <button
                        ...
                        className={`w-full rounded-lg border p-3 text-left transition-all flex items-center justify-between ${
                          selectedProject?.id === project.id
                            ...
                            : 'border-line-muted bg-surface-hover hover:bg-surface-raised hover:border-line'
                        }`}
                      >
                        <div className="truncate pr-4">
                          <div className="font-mono text-[10px] font-bold text-text-primary truncate">{project.name}</div>
                          <div className="font-mono text-[8px] text-text-secondary truncate">{project.scope}</div>
                        </div>
                        {project.rewards && (
            ...
            <div className="space-y-6">
              <div>
                ...
                <div className="grid grid-cols-2 gap-3" role="radiogroup" aria-label="Scan preset mode">
                  <button
                    type="button"
                    onClick={() => {
                      setScanMode('safe');
                    ...
                    className={`flex flex-col items-start rounded-xl border p-3.5 text-left transition-all ${
                      scanMode === 'safe'
                        ? 'border-accent bg-accent/10 shadow-glow-accent-sm text-text-primary'
                        : 'border-line-muted bg-surface-hover text-text-secondary hover:bg-surface-raised'
                    }`}
                  >
                    <span className="text-xs font-black uppercase tracking-wider text-text-primary">Passive Safe</span>
                  ...
                  >
                    <span className="text-xs font-black uppercase tracking-wider text-text-primary">Active Vulnerability</span>
                    <span className="mt-1 text-[9px] leading-relaxed opacity-60 font-mono">
              ...
              <div className="space-y-3 rounded-xl border border-line-muted bg-surface-2 p-4">
                <div className="flex items-center justify-between font-mono text-[9px] uppercase tracking-wider text-text-secondary">
                  <span>Depth: Level <span className="tabular-nums">{scanDepth}</span></span>
                  <span>Concurrency: <span className="tabular-nums">{scanConcurrency}</span> workers</span>
                </div>
          ...
          <div className="mt-8 border-t border-line-muted pt-6 flex items-center justify-end">
            <button
              type="button"
              onClick={onStartScan}
