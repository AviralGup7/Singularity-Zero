            ...
            <div className="max-h-96 overflow-y-auto scrollbar-cyber">
              <table className="recon-table w-full text-left text-xs">
                <thead className="sticky top-0 bg-panel shadow-sm">
                  <tr className="text-muted border-b border-line uppercase tracking-tighter">
                        ...
                        {url?.tech && url.tech.length > 0 ? (
                          <div className="flex flex-wrap gap-1">
                            {url.tech.map((t, i) => (
                              ...
                              <span key={i} className="bg-surface-hover px-1.5 py-0.5 rounded text-[9px] border border-line">{t}</span>
                            ))}
                          </div>
                        ) : '—'}
                      </td>
          ...
          {parameters.length === 0 ? (
            ...
            <div className="max-h-96 overflow-y-auto scrollbar-cyber space-y-2">
              {(parameters || []).map((param, idx) => (
                <div key={idx} className="recon-param-item bg-surface-hover p-3 rounded-lg border border-line group">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {(param?.parameters || []).map((p, i) => (
                      ...
                      <span key={i} className="bg-surface-2 text-accent/80 border border-accent/20 px-2 py-0.5 rounded text-[10px] font-mono group-hover:border-accent/40 transition-colors">{p}</span>
                    ))}
                  </div>
                </div>
              ))}
