...
function extractMentions(text: string, members: string[]): string[] {
  const mentions: string[] = [];
  const ranges = findMentionRanges(text, members);
  for (const range of ranges) {
    const raw = text.slice(range.start + 1, range.end);
    const found = members.find(m => m.toLowerCase() === raw.toLowerCase());
...
function useFindingComments(findingId: string, targetName?: string, runId?: string) {
  ...
  const teamMembers = useMemo(() => {
    ...
    collaboration.presence.forEach(p => {
      if (p.analyst_name) names.add(p.analyst_name);
    });
    if (currentUser?.name) names.add(currentUser.name);
    if (currentUser?.role) names.add(currentUser.role.charAt(0).toUpperCase() + currentUser.role.slice(1));
    if (names.size === 0) return FALLBACK_TEAM_MEMBERS;
    return Array.from(names);
  }, [collaboration.presence, currentUser]);
  ...
  }, [collaboration, newComment, teamMembers]);
  ...
  }, [collaboration, replyText, teamMembers]);
  ...
  }, [collaboration, editingText, teamMembers]);
