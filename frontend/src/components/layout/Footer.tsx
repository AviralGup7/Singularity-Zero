  ...
  return (
    ...
    >
      <div className="footer-health">
        <span className="footer-muted" aria-label={`Version ${appVersion}`}>
          v{appVersion}
        </span>
        ...
        <span className="flex items-center gap-1.5" role="status" aria-live="polite">
          ...
          <span className={isOnline ? 'footer-muted' : 'text-bad'}>
        ...
        {liveConnectionState && (
          <>
            ...
            <span className="flex items-center gap-1.5" role="status" aria-live="polite" aria-label={`Connection: ${connectionLabel(liveConnectionState)}`}>
              ...
              <span className="footer-muted">{connectionLabel(liveConnectionState)}</span>
            </span>
          </>
        )}
      </div>
      <div className="footer-health">
        <button
          type="button"
          className="footer-muted hover:text-accent transition-colors duration-200 group"
          onClick={onRefresh}
        ...
        <span className="w-px h-3 bg-line" aria-hidden="true" />
        <span className="footer-muted" aria-label={`Copyright ${new Date().getFullYear()}`}>
