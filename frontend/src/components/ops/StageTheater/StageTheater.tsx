...
import type { StageGraph, StageTheaterNode } from '@/lib/stageTheaterUtils';
import { layoutStageDag } from '@/lib/stageTheaterUtils';
import { AMBIENT_LOG_LINES } from './constants';
...
export function StageTheater({ nodes, graph, className }: StageTheaterProps) {
  const { state: visualState } = useVisual();

  const layout = useMemo(() => {
    const ids = nodes.map((node) => node.id);
    return layoutStageDag(ids, graph?.edges);
  }, [nodes, graph]);

  const dimensions = useMemo(() => {
    const maxBreadth = Math.max(...layout.levels.map((level) => level.length), 1);
    const levelCount = Math.max(layout.levels.length, 1);
    const paddingX = 150;
    const paddingY = 118;
  ...
  }, [nodes, layout.levels, dimensions.height, dimensions.paddingX, dimensions.paddingY, dimensions.width]);
    ...
    return layout.edges
      .map(([sourceId, targetId]) => {
        ...
        return {
          ...
          d,
          isFlowing: source.status === 'running' || target.status === 'running' || target.status === 'ready',
  ...
  const focusNodeId = useMemo(() => {
    ...
    return positionedNodes.find((node) => node.status === 'error')?.id
      ?? positionedNodes.find((node) => node.status === 'ready')?.id
      ?? null;
  }, [positionedNodes]);
