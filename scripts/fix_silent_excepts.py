...
def has_logger(content: str) -> bool:
    return bool(re.search(r'^\s*(import logging|logger\s*=\s*logging\.getLogger)', content, re.MULTILINE))
...
def _classify_context(content: str, pos: int) -> str:
    ...
    if any(w in context for w in ["close", "cleanup", "shutdown", "__del__", "destructor", "release", "detach", "finaliz"]):
        return "cleanup"
