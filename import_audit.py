        ...
        for node in nodes:
            if isinstance(node, ast.Import):
                ...
                if level > 0:
                    ...
                    abs_parts = cur_module_dotted.split(".")[:-level] if cur_module_dotted.split(".") else []
                    abs_parts = abs_parts + ["__package__placeholder__"]
                    actual_module_parts = cur_module_dotted.split(".")[:max(0, len(cur_module_dotted.split("."))-level)]
                ...
                for alias in node.names:
                    ...
                    check_import(py, node.lineno, full, cur_module_dotted, module_map, "from", relative_level=level, relative_module=module_prefix)
...
def check_import(py, lineno, full_parts, cur_module_dotted, module_map, kind, relative_level=0, relative_module=""):
    ...
    stdlib_modules = {
        "abc", "argparse", "asyncio", "base64", "binascii", "collections", "copy",
        "csv", "datetime", "difflib", "enum", "fnmatch", "functools", "getopt",
        "glob", "hashlib", "heapq", "hmac", "html", "http", "inspect", "io",
        "itertools", "json", "logging", "math", "multiprocessing", "os", "pathlib",
        "pickle", "platform", "pprint", "random", "re", "secrets", "shutil",
        "signal", "socket", "sqlite3", "ssl", "statistics", "string", "struct",
        "subprocess", "sys", "tempfile", "textwrap", "threading", "time",
        "traceback", "types", "typing", "uuid", "urllib", "warnings", "wave",
        "xml", "zipfile", "zoneinfo", "dataclasses", "typing_extensions",
        "contextlib", "decimal", "queue", "weakref", "array", "bisect", "codecs",
        "codeop", "collections", "concurrent", "configparser", "contextvars",
        "ctypes", "curses", "dataclasses", "datetime", "email", "ensurepip",
        "errno", "faulthandler", "fcntl", "filecmp", "fileinput", "formatter",
        "graphlib", "grp", "gzip", "hashlib", "heapq", "imaplib", "imghdr",
        "imp", "importlib", "inspect", "ipaddress", "keyword", "linecache",
        "locale", "lzma", "mailbox", "mimetypes", "mmap", "modulefinder",
        "netrc", "nis", "nntplib", "optparse", "parser", "pdb", "pickle",
        "pip", "pipes", "pkgutil", "platform", "plistlib", "posix", "posixpath",
        "pprint", "profile", "pstats", "pty", "pwd", "py_compile", "pyclbr",
        "pydoc", "queue", "quopri", "random", "readline", "reprlib",
        ...
        "xml", "xmlrpc", "zipapp", "zipfile", "zlib",
        "_thread", "__future__",
        # Third-party commonly used (non-exhaustive)
        "requests", "httpx", "httptools", "starlette", "fastapi", "uvicorn",
        "flask", "django", "sqlalchemy", "pydantic", "numpy", "pandas",
        "scrapy", "selenium", "playwright", "beautifulsoup4", "bs4", "lxml",
        "celery", "redis", "pika", "kafka", "grpc", "protobuf", "msgpack",
        "cryptography", "pycryptodome", "bcrypt", "argon2", "passlib",
        "psycopg2", "mysqlclient", "paramiko", "fabric", "ansible",
        "boto3", "botocore", "google", "azure", "pyjwt", "jwt",
        "croniter", "python_dateutil", "dateutil", "websockets",
        "aiohttp", "aiofiles", "pyyaml", "yaml", "toml", "configparser",
        "prometheus_client", "opentelemetry", "sentry_sdk", "loguru",
        "typing_extensions", "pytest", "coverage", "mypy",
        "watchdog", "psutil", "dnspython", "scapy", "mitmproxy",
        "h2", "hpack", "hyperframe", "graphql_core", "graphene",
        "nltk", "spacy", "transformers", "torch", "tensorflow",
        "scikit_learn", "xgboost", "lightgbm", "catboost",
        "pydash", "toolz", "cytoolz", "boltons", "more_itertools",
        "stdnum", "phonenumbers", "email_validator", "pydantic_settings",
    }
    ...
    if target is None:
        ...
        if kind == "import" and len(full_parts) == 1:
            issues.append((str(py), lineno, "Import of unknown module/package '{}' (no matching src/ file or package)".format(".".join(full_parts))))
        else:
            issues.append((str(py), lineno, "Import of unknown module/package '{}' (no matching src/ file or package)".format(".".join(full_parts))))
