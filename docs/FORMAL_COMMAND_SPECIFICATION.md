...
6. **Region Consistency (Invariant I36)**: Commands are admitted only on the current leader home. `WALReplicationRelay` must not commit peer settlements. Budget is only on `P-0000`. A lease reserved in region A cannot be settled in B without a fenced transfer. In-flight attempts do not migrate. Healing uses `placement_version`, never LWW.
