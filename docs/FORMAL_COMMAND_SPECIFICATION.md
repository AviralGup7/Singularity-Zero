...
7. **Authority Transfer Fence (Invariant I37)**: `initiate_transfer` fences the old home (`OWNED → FENCED`) and mints `AuthorityEpoch` + `FenceToken` + `AuthorityRevision`. It does not move `partition_home`. During FENCED `propose_and_commit` and ticket issue fail closed. `activate_ownership` is the only cutover (`FENCED → OWNED` on the pending home). Old I30 ticket revisions cannot be consumed. An I30-invalid ticket cannot be resurrected by transfer (`assert_tickets_not_resurrected` / proof graph). Wrong fence token cannot activate.
