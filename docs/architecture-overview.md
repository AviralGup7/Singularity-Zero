...
| **Formal Invariants Verification** | `src/core/frontier/invariant_checker.py`, `tests/unit/test_formal_invariants.py` | Automated invariant audit suite evaluating all 21 formal system invariants ($I_1$–$I_{21}$) including cryptographic state commitment, lease linearization, authority uniqueness, and projection recovery. |
