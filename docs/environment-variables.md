...
| `GRAPHGEN_STRICT` | boolean | `true` | Fail-closed `GraphGenerationMismatch` when a stored *declared* GraphGenID differs. Hash is computed before tool-availability prune; empty stored id (pre-MVR checkpoint) skips. |
