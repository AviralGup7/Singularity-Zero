...
Canonical invariant set is **I1–I29** plus cross-subsystem **I30–I37** (see §6). The 6-level authority hierarchy is Axiom 1 (not a 7-layer or 16-invariant count).
...
30. **I29 (Universal Scope-Derived Network Egress Authority)**: Outbound network destinations are derived strictly from `ScopeToken` and enforced across every network-capable execution primitive (HTTP clients, raw TCP/UDP sockets, asyncio streams, custom HTTP/2, raw WebSockets, headless browser CDP/navigation, and subprocesses). No transport operates uncataloged; cloud metadata services (`169.254.169.254`, `metadata.google.internal`, etc.) are unconditionally denied.
31. **I30 (Universal Authorization Causality)**: An `AuthorizedExecutionTicket` exists only if it binds `ScopeToken` hash, an atomic budget reservation recorded on `HuntBudgetEnforcer`, an authority revision, and a `command_id`. Consume fails if the reservation is not in the ledger. Every execution path (stage pipeline execution, exploit campaigns, standalone `SafeExploiter`) must acquire and consume a valid ticket prior to execution and authoritatively settle quota.
...
36. **I35 (Recovery Protocol)**: Every durable object has a declared authoritative source, reconstructibility, determinism, and idempotency in `src/core/frontier/recovery_protocol.py`. Recovery is a state machine (`UNINITIALIZED → … → VERIFY_INVARIANTS → READY | FAIL_CLOSED | FRESH`), not a second failure table. Snapshot vs WAL disagreement, truncated WAL, semantically old snapshot, schema newer than the reader, outbox/FSM skew, delivery ahead of outbox, crash between WAL commit and outbox append, crash between outbox and delivery, crash during compensation, and recovered artifacts that fail a prerequisite invariant (I30/I31/I32/I33) each have one resolution. Partition plane is fail-closed; FrontierWAL scan journal may keep a STALE snapshot. Checkpoints and DeliveryLedger are never authority.
...
38. **I37 (Authority Transfer Fence)**: Home moves only as `OWNED → FENCED → OWNED`. `initiate_transfer` seals the old home and mints `AuthorityEpoch` + `FenceToken` + `AuthorityRevision`; it does **not** move `partition_home`. During FENCED nobody may mutate (I34). `activate_ownership` is the only cutover: new home writes, old token/revision die. Integrated directly into `ProactiveMigrationHandler` actor evacuation loop and `PlacementAuthority`. Tickets bound to the previous I30 `authority_revision` cannot be consumed. An I30-invalid ticket cannot be resurrected by a successful transfer (`assert_tickets_not_resurrected`). A and B cannot both believe they are leader. Machine-readable: `src/core/frontier/authority_transfer.py`.
