...
38. **I37 (Authority Transfer Fence)**: Home moves only as `OWNED → FENCED → OWNED`. `initiate_transfer` seals the old home and mints `AuthorityEpoch` + `FenceToken` + `AuthorityRevision`; it does **not** move `partition_home`. During FENCED nobody may mutate (I34). `activate_ownership` is the only cutover: new home writes, old token/revision die. Tickets bound to the previous I30 `authority_revision` cannot be consumed. An I30-invalid ticket cannot be resurrected by a successful transfer (`assert_tickets_not_resurrected`). A and B cannot both believe they are leader. Machine-readable: `src/core/frontier/authority_transfer.py`.
