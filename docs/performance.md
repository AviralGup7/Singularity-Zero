...
- **Metric**: `cache_read_latency` > 10ms -> **Action**: Trigger **Bloom-Aware Smart Cache Routing** in `CacheManager` to bypass slow L2 (SQLite/Redis) or L3 (File) backend lookups on cache misses using the active Bloom filter.
