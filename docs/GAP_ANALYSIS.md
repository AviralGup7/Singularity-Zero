...
| **I37 Authority Transfer** | Fenced `OWNED -> FENCED -> OWNED` zero-dual-writer authority cutover wired into `ProactiveMigrationHandler` actor evacuation loop and `PlacementAuthority` partition leadership. | `test_authority_transfer.py`, `test_formal_invariants.py` |
