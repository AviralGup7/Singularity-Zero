...
| **Execution Authorization** | `ExecutionAuthorizer.authorize` requires `reserve_with_identity` (I30). `consume_ticket` commits I28 `commit_requests(1)`. Admit path is authorize → consume → `ProcessSandbox.check_egress`. `ProcessSandbox.run` unused. Nuclei/validation/active_scan may still `authorize()` again (**double reserve** leftover). HMAC: no published fallback; missing env key → process-local random (dies across restart). | `test_global_invariants.py`, `test_atlas_holes.py` |
