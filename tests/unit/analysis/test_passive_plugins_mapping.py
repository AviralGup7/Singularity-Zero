...
class TestPassivePluginsMapping(unittest.TestCase):
    ...
    def test_exposure_related_plugins_are_mapped(self) -> None:
        ...
        for k in expected_keys:
            self.assertIn(
                k,
                exposure_keys,
                f"Expected exposure plugin '{k}' is missing or not grouped under 'exposure'",
            )
    ...
    def test_idor_logic_diff_plugins_are_mapped(self) -> None:
        ...
        for k in expected_idor:
            self.assertIn(
                k, idor_keys, f"Expected IDOR plugin '{k}' is missing or not grouped under 'idor'"
            )
        ...
        for k in expected_logic:
            self.assertIn(
                k,
                logic_keys,
                f"Expected logic plugin '{k}' is missing or not grouped under 'logic'",
            )

        # Verify specific diff-related plugins exist and are not overlapping (they belong to distinct categories/groups)
        diff_specs = [
            p for p in PASSIVE_PLUGIN_SPECS if "diff" in p.key or "diff" in p.description.lower()
        ]
        ...
        self.assertEqual(
            len(diff_keys), len(set(diff_keys)), f"Duplicate/overlapping diff plugins: {diff_keys}"
        )
        ...
        for k in expected_diffs:
            self.assertIn(
                k,
                diff_keys,
                f"Expected diff-related plugin '{k}' is missing or not identified by diff characteristics",
            )
