...
def test_prioritized_broker_strict_ordering() -> None:
    ...
    broker.publish(
        TelemetryEvent(
            event_id="e_debug", qos=QoSClass.P4_DEBUG, topic="debug", payload={"log": "trace"}
        )
    )
    broker.publish(
        TelemetryEvent(
            event_id="e_metric", qos=QoSClass.P3_TELEMETRY, topic="rps", payload={"rps": 100}
        )
    )
    broker.publish(
        TelemetryEvent(
            event_id="e_finding", qos=QoSClass.P2_FINDINGS, topic="findings", payload={"id": "f1"}
        )
    )
    broker.publish(
        TelemetryEvent(
            event_id="e_life", qos=QoSClass.P1_LIFECYCLE, topic="stage", payload={"stage": "recon"}
        )
    )
    broker.publish(
        TelemetryEvent(
            event_id="e_ctrl", qos=QoSClass.P0_CONTROL, topic="emergency", payload={"stop": True}
        )
    )
...
def test_prioritized_broker_coalescing_findings() -> None:
    ...
    broker.publish(
        TelemetryEvent(
            event_id="f_1_v1",
            qos=QoSClass.P2_FINDINGS,
            topic="findings",
            dedup_key="finding:sqli:users",
...
def test_prioritized_broker_debug_load_shedding() -> None:
    ...
    for i in range(5):
        broker.publish(
            TelemetryEvent(
                event_id=f"dbg_{i}", qos=QoSClass.P4_DEBUG, topic="debug", payload={"line": i}
            )
        )
